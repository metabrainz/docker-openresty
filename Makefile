# based on https://github.com/microscaling/microscaling/blob/master/Makefile
# example given at https://microbadger.com/labels
#
default: build
...
DOCKER_IMAGE ?= metabrainz/docker-openresty
...
TAGGED = $(strip $(shell git rev-parse "$(CODE_VERSION)" >/dev/null 2>&1 && echo "ok"))
ifneq (x$(TAGGED), xok)
$(error echo No matching tag: $(CODE_VERSION))
endif
